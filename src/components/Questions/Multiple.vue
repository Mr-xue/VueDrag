    ...
    <div class="multiple question">
        ...
        <div class="show_question" v-if="!isEdit">
            ...
            <div class="question_header">
                <i v-if="multiple.required">*</i>
                <span class="title">{{ multiple.title }}</span>
            </div>
            <ul class="choice">
                <li v-for="(item,index) in multiple.choice">
        ...
        <div class="edit_question" v-else>
            ...
            <div class="question_header">
                <i v-if="multiple.required">*</i>
                <input type="text" placeholder="请输入单选题目" class="title" v-model="multiple.title">
            </div>
            <ul class="choice">
                <draggable
                v-model="multiple.choice" 
                :options="dragOptions" 
                @start="drag=true" 
                @end="drag=false">
                    <li v-for="(item,index) in multiple.choice">
            ...
            <div class="bottom">
                <div class="left">
                    ...
                    <span :class="multiple.choice[multiple.choice.length-1].type=='other'? 'addOther active':'addOther'" @click="addOther">添加其他</span>
                </div>
                <div class="right">
                   <div class="elective" @click="multiple.required = !multiple.required">
                        <i class="iconfont icon-fangxingweixuanzhong" v-if="multiple.required"></i>
...
export default {
    ...
    methods:{
        ...
        addChoice (){
            let len = this.multiple.choice.length;
            let newCho = {
                type:'normal',
                title: "选项"+len, 
            }
            if(this.multiple.choice[len-1].type!='other'){
                this.multiple.choice.push(newCho);
            }else{
                this.multiple.choice.splice(len-1,0,newCho)
            }
        },
        removeChoice (e){
            this.multiple.choice.splice(e,1)
        },
        addOther (){
            let len = this.multiple.choice.length;
            if(this.multiple.choice[len-1].type!='other'){
